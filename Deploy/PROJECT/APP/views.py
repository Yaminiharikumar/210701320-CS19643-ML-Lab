...
Model1 = joblib.load('/Users/swethachakkaravarthi/Downloads/PREGANANCY-ML-MINI-MI0047/Deploy/PROJECT/APP/NSP1.pkl')  
...
def Deploy_9(request): 
    if request.method == "POST":
        ...
        if output == 0:
            return render(request, 'output.html', {"prediction_text": "NORMAL CONDITIONS"})
        elif output == 1:
            return render(request, 'output.html', {"prediction_text": "SUSPECT - SCISSORION"})
        
        elif output == 2:
            return render(request, 'output.html', {"prediction_text": "PATHOLOGY - ABNORMAL CONDITIONS"})
    else:
